...
def train_net(model, optimizer, train_dl, val_dl, num_epochs, scheduler = None, plotting=True, verbose = True):
    ...
    try:
            ...
            #     try:
                ...
                if val_loss.item() < bestloss:
                    ...
                    if plotting:
                        ...
                        plt.legend()
                        plt.savefig(f'plots/rolling/sxs_mm_hist_rolling_summed.png', dpi=300)
                        plt.close()
                        ...
                        plt.savefig(f'plots/rolling/sxs_worst_recon_mm_summed.png', dpi=300)
                ...
                if (epoch % 10 == 0):
                    torch.save(bestmodel_weights, f'models/rolling/decoder_kfold.pt')
                    if plotting:
                        plt.figure()
        ...
        # Save the best model
    # if plotting:
    torch.save(bestmodel_weights, f'models/decoder_kfold.pt')
...
if train:
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_test_idx)):
        ...
        torch.save(model.state_dict(), f'models/rolling/decoder_{fold}.pt')
        # torch.save({'train_losses': train_losses, 'val_losses': val_losses, 'val_mms': mm_history}, f'models/rolling/decoder_{fold}_losses.pt')

        print(f'Fold {fold+1} completed. Best loss: {best_loss:.2e}, Mean MM: {mean_mm:.2e}')



fold_models = [torch.load(f'models/rolling/decoder_{fold}.pt', map_location=device) for fold in range(k)]
...
with torch.no_grad():
    ...
    plt.savefig(f'plots/sxs_mm_hist_test.png', dpi=300)
    plt.close()
    ...
    plt.savefig(f'plots/sxs_mm_vs_q_test.png', dpi=300)
    plt.close()

print(f'\nFinal test loss: {test_loss.item():.2e}')
# Save the averaged model
torch.save(avg_model.state_dict(), f'models/decoder_kfold.pt')
